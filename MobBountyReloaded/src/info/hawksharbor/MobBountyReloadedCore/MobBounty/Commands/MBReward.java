...
public class MBReward implements CommandExecutor
{
	...
	public MBReward(info.hawksharbor.MobBountyReloadedCore.MobBounty.MobBountyReloaded plugin)
	{
		_plugin = plugin;
	}
			...
			String label, String[] args)
	{
		...
		if (_plugin.getAPIManager().getPermissionsManager()
				.hasPermission(player, "mbr.command.mbr"))
		{
			if (args.length == 2)
			{
				...
				if (mob != null)
				{
					if (args[1].matches("[-]?[0-9]+([.][0-9]+)?"))
					{
						...
						String message = _plugin.getAPIManager()
								.getLocaleManager().getString("MBRChange");
						if (message != null)
						{
